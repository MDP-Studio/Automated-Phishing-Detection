...
| **Platform-admin browser access is user-bound and fail-closed** | `main.py::_passkey_admin_context`, `main.py::require_admin_access`, `tests/unit/test_saas_api.py` | S-TB6, E-TB6: a SaaS session reaches `/admin` only when the user has an owner/admin role, appears in the independent platform-admin allowlist, has a registered passkey, and completed a fresh step-up; an empty allowlist disables the bridge |
| **Operational alert payloads use closed, privacy-minimized schemas** | `src/automation/operational_alerts.py`, `tests/unit/test_operational_alerts.py` | I-TB4, I-TB6: outbound and local operational events reject unknown fields and omit mailbox content, addresses, subjects, URLs, client IPs, raw user IDs, and raw tenant IDs |
...
**Status: MITIGATED.**
**Description:** All state-changing and information-disclosing analyst endpoints in `main.py` (`/api/feedback`, `/api/feedback/retrain`, `/api/accounts/*`, `/api/detonate-url`, `/api/diagnose`, `/api/system-status`, `/admin/api/overview`, `/api/monitor/email/{id}`, `/api/analyze/upload`, `/api/config`, `/api/monitor/alerts`) use the shared admin access dependency. Existing API clients can use bearer auth, and browser analysts can use the signed analyst session plus its CSRF boundary. A user-bound browser bridge is also available when `PHISHANALYZE_ADMIN_AUTH_MODE=token_or_owner_passkey`: the signed SaaS session must belong to the same organization, the user must have an owner/admin role, their normalized email must appear in `PHISHANALYZE_ADMIN_USER_EMAILS`, they must have a registered passkey, and their database-backed step-up must still be fresh. State-changing bridge requests use the SaaS CSRF boundary. Tenant ownership alone never grants platform access, and an empty allowlist disables the bridge. Failed analyst token logins and failed SaaS email/password logins are throttled and emit only privacy-minimized threshold events. The same token (`ANALYST_API_TOKEN` env var) remains enforced by the existing `src/feedback/feedback_api.py` router for compatibility. Normal user routes under `/api/saas/*` use separate signed user sessions, CSRF protection, tenant-scoped `org_id` queries, and plan gates; `/analyze`, `/dashboard`, and `/monitor` are public user shells that show only the signed-in user's workspace data. Stripe subscription webhooks are the deliberate exception to user CSRF because they are server-to-server calls; `/api/stripe/webhook` verifies `Stripe-Signature` with `STRIPE_WEBHOOK_SECRET` before mutating subscription state. `SAAS_PUBLIC_SIGNUP_ENABLED=false` by default, so deployments do not accept visitor email uploads unless the operator deliberately enables signup. `PUBLIC_DEMO_MODE=true` only opens `/demo`, `/api/demo/status`, and `/api/demo/plans`; those routes serve fixed sample content, explicit capability flags, and non-secret plan metadata, not mailbox data, uploaded emails, feedback labels, owner account management, URL detonation, or paid API-backed analysis. Additionally, `run_server()` defaults to `127.0.0.1` and refuses to start on a non-loopback host without `ANALYST_API_TOKEN`.

### R2 — Analyst is a single trust principal
**Severity:** high. **Likelihood:** low (depends on operator).
**Description:** No separation of duties exists between analysts. Any bearer-token or analyst-session user can re-label any verdict, and compatibility authentication does not bind actions to an individual. The passkey bridge identifies the browser user but does not yet add per-action actor attribution to the legacy feedback path.
**Compensating control:** Treat the feedback DB as an audit artifact, back it up off-host, keep the compatibility token path internal, and run with a small trusted analyst pool only.
...
The pipeline stores email metadata — sender, recipient, subject, body excerpt, verdict, IOCs — in `data/results.jsonl`, `data/alerts.jsonl`, the SQLAlchemy feedback DB, and the sender profile DB. Under the **Australian Privacy Act 1988** and the **EU GDPR**, this is regulated personal information processing. The risk is legal/regulatory, not a confidentiality breach in the security sense, so it's tracked separately from the STRIDE residual risks above. `data/operational_alerts.jsonl` uses a separate closed schema that rejects mailbox content and direct identifiers; it includes an optional tenant pseudonym only when `OPERATIONAL_ALERT_HASH_KEY` is configured.

| Risk | Severity | Status | Notes |
|---|---|---|---|
| **P1 — Indefinite retention of runtime artifacts** | medium (legal exposure) | **MITIGATED** | `python main.py purge --target all --older-than 30` covers `results.jsonl`, phishing alerts, privacy-minimized operational alerts, `feedback.db`, SaaS rows, and `sender_profiles.db`. Default retention is 30 days, configurable via `data_retention_days` in `config.yaml` or `DATA_RETENTION_DAYS` env var. Operators should run it on a daily cron. The strongest JSONL property the test suite locks is that, after purge, no row remains with a timestamp older than the cutoff. |
...
- **The analyst dashboard and mailbox monitor are not multi-tenant.** `/app` user manual scans are organization-scoped, but the legacy analyst dashboard, feedback DB, credential vault, and mailbox monitor remain platform-operator surfaces. A tenant owner can reach them only when separately listed in the platform-admin allowlist and verified with a fresh passkey step-up.
