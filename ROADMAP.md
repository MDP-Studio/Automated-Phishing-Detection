...
| **Payment relevance gate** — high-recall classifier that labels `invoice`, `payment_request`, `bank_detail_change`, `receipt`, `billing_notice`, `non_payment`, and `unknown`; clear `non_payment` skips deep PayShield analysis while `unknown` still scans. | `src/analyzers/payment_relevance.py`, `src/orchestrator/pipeline.py`, SaaS mailbox scan-now |
| **Payment relevance ML monitor lane** - `payment_relevance` dataset labels, rule prelabeling with a manual review queue, relevance eval false-negative metrics, and a TF-IDF + logistic regression model target that reports monitor-only sidecar evidence without controlling skips. | `src/eval/payment_dataset.py`, `src/eval/payment_relevance_eval.py`, `src/ml/payment_classifier.py`, `src/analyzers/payment_relevance.py`, `scripts/payment_relevance_eval.py` |
...
| **Full runtime retention policy** - `purge --target all` now covers results, phishing alerts, privacy-minimized operational alerts, feedback labels, SaaS rows, and sender profiles by age. Data-subject erasure applies to stores that can contain the supplied address. | `src/automation/retention.py`, `main.py purge` |
...
| **Passkey-backed owner admin bridge** - `/admin` and its protected APIs accept a valid SaaS session only for a platform-allowlisted owner/admin with a registered passkey and fresh step-up. Tenant owner role alone cannot grant global admin. Existing bearer and analyst-token sessions remain compatible; SaaS-authenticated mutations use the SaaS CSRF boundary. | `main.py`, `src/saas/auth.py`, `src/saas/database.py`, `static/shared.js`, `docs/adr/0003-operational-alerts-and-passkey-admin-bridge.md` |
| **Privacy-minimized operational alerts** - analyzer circuit transitions, repeated concrete campaign signals, high PayShield payment risk, and login throttle thresholds write a closed-schema JSONL event and can POST the same event to an operations webhook. Raw mailbox content, addresses, subjects, URLs, client IPs, user IDs, and tenant IDs are rejected or omitted. | `src/automation/operational_alerts.py`, `src/analyzers/clients/base_client.py`, `main.py`, `docs/operational-alerting.md` |
| **Current advisory-free dependency lock** - the July 2026 refresh removed 26 advisories from seven existing packages, raised their minimum safe versions, regenerated all hashes, and passed the full suite against the upgraded environment. | `requirements.txt`, `requirements.lock` |
...
| 1,369 tests (82 test modules) | unit + integration |
